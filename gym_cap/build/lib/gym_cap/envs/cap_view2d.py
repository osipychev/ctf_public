...
class CaptureView2D:
    ...
    def human_move(self, env, team2):
        moves_recorded = 0
        human_move_list = [0]*(NUM_RED+NUM_UAV)
        tile_w = self.SCREEN_W/len(env)
        tile_h = self.SCREEN_H/len(env[0])
        map_h = len(env[0])
        map_w = len(env)
        selected = (0, 0)
        isSelected = -1
        while moves_recorded < NUM_RED+NUM_UAV:
            ev = pygame.event.get()
            ...
            for event in ev:
                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    ...
                    if env[x][y] == TEAM2_UAV:
                        # Reset previously selected to normal color
                        if isSelected >= 0:
                    ...
                    # Moving unit up
                        ...
                        moves_recorded+=1
                        team2[isSelected].moveSelected = True
