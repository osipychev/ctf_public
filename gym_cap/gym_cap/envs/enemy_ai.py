...
class EnemyAI:
    def patrol(map_only, team2):
        ...
        for row in range(len(map_only)):
            if (map_only[row][0] == MapConst.TEAM1_BACKGROUND and
                map_only[row+1][0] == MapConst.TEAM2_BACKGROUND):
                border = row + 1
                break
        ...
        for entity in team2:
            ...
            if y > border + 2:
                ...
                if map_only[y-1][x] != MapConst.TEAM2_BACKGROUND:
                    action = 1
            elif y < border + 1:
                ...
                if (x >= len(map_only[0])-1 or
                    map_only[y][x+1] != MapConst.TEAM2_BACKGROUND):
                    action = 2
            elif y == border + 2:
